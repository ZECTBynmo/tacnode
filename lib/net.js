...
var IOWatcher2 = require('_io_watcher').IOWatcher;
...
Socket.prototype._onIO = function(watcher, readable, writable) {
  if (readable) {
    this._onReadable();
  }
...
Socket.prototype.open = function(fd, type) {
  ...
  this.type = type || null;
  this.fd = fd;
  this._readWatcher = new IOWatcher2(this);
  this._writeWatcher = new IOWatcher2(this);
...
Socket.prototype.connect = function() {
  ...
  if (port === false) {
    ...
    self.type = 'unix';
    self.fd = socket('unix');
    self._readWatcher = new IOWatcher2(self);
    self._writeWatcher = new IOWatcher2(self);
...
Socket.prototype.destroy = function(exception) {
  ...
  if (this._writeWatcher) {
    this._writeWatcher.free();
    this._writeWatcher = null;
  }

  if (this._readWatcher) {
    this._readWatcher.free();
...
Server.prototype._getFD = function(type) {
  ...
  this.type = type;
  this.fd = socket(type);
  this.watcher = new IOWatcher2(this);
};
